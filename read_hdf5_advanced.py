...
def save_camera_data_from_hdf5(
    ...
    save_depth_arrays: bool = False,
    save_cam2world: bool = False,
    ...
    intrinsics_dir = os.path.join(output_dir, "intrinsics")
    depths_dir = os.path.join(output_dir, "depths")
    depth_arrays_dir = os.path.join(output_dir, "depth_arrays")
    cam2world_dir = os.path.join(output_dir, "cam2world")
    videos_dir = os.path.join(output_dir, "videos")

    def _cam_dir(base_dir: str, cam_name: str) -> str:
        path = os.path.join(base_dir, cam_name)
        os.makedirs(path, exist_ok=True)
        return path

    base_dirs = [images_dir, extrinsics_dir, intrinsics_dir, depths_dir]
    if save_depth_arrays:
        base_dirs.append(depth_arrays_dir)
    if save_cam2world:
        base_dirs.append(cam2world_dir)
    for base_dir in base_dirs:
        os.makedirs(base_dir, exist_ok=True)
        for cam_name in cameras:
            os.makedirs(os.path.join(base_dir, cam_name), exist_ok=True)
    if save_videos:
        os.makedirs(videos_dir, exist_ok=True)
        for cam_name in cameras:
            os.makedirs(os.path.join(videos_dir, cam_name), exist_ok=True)
    ...
    video_writers: Dict[str, subprocess.Popen] = {}
    video_shapes: Dict[str, Tuple[int, int]] = {}
    saved_intrinsics = set()
    ffmpeg_available = shutil.which("ffmpeg") is not None
    if save_videos and not ffmpeg_available:
    ...
    def _start_video_writer(cam_name: str, image_bgr: np.ndarray) -> subprocess.Popen:
        ...
        video_shapes[cam_name] = (h, w)
        print(f"开始保存视频: {video_path} ({w}x{h}, {video_fps} FPS)")
        return proc
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            for cam_name in cameras:
                ...
                if (save_rgb or save_videos) and "rgb" in cam_g:
                    ...
                    if decoded_img is not None:
                        ...
                        if save_videos:
                            if decoded_img.ndim != 3 or decoded_img.shape[2] != 3:
                                ...
                                expected_h, expected_w = video_shapes[cam_name]
                                if decoded_img.shape[:2] != (expected_h, expected_w):
                                    raise ValueError(
                                        f"{cam_name} 视频帧尺寸变化: "
                                        f"expected {(expected_h, expected_w)}, got {decoded_img.shape[:2]}"
                                    )
                                video_writers[cam_name].stdin.write(np.ascontiguousarray(decoded_img).tobytes())

                if save_extrinsic and "extrinsic_cv" in cam_g:
                    ext = cam_g["extrinsic_cv"][frame_idx]
                    ext_filename = f"frame_{frame_idx:06d}.npy"
                    np.save(os.path.join(_cam_dir(extrinsics_dir, cam_name), ext_filename), ext)
                    saved_count += 1

                if save_intrinsic and "intrinsic_cv" in cam_g and cam_name not in saved_intrinsics:
                    it = cam_g["intrinsic_cv"][frame_idx]
                    np.save(os.path.join(_cam_dir(intrinsics_dir, cam_name), "intrinsic.npy"), it)
                    saved_intrinsics.add(cam_name)
                    saved_count += 1

                if save_cam2world and "cam2world_gl" in cam_g:
                    c2w = cam_g["cam2world_gl"][frame_idx]
                    c2w_filename = f"frame_{frame_idx:06d}.npy"
                    np.save(os.path.join(_cam_dir(cam2world_dir, cam_name), c2w_filename), c2w)
                    saved_count += 1
                ...
                if save_depth and "depth" in cam_g:
                    ...
                    if depth is not None:
                        ...
                        Image.fromarray(depth_array).save(os.path.join(_cam_dir(depths_dir, cam_name), depth_filename))
                        if save_depth_arrays:
                            depth_npy_filename = f"frame_{frame_idx:06d}.npy"
                            np.save(os.path.join(_cam_dir(depth_arrays_dir, cam_name), depth_npy_filename), depth)
                            saved_count += 1
                        saved_count += 1
    finally:
        for cam_name, proc in video_writers.items():
            if proc.stdin:
                proc.stdin.close()
            if proc.wait() != 0:
                print(f"WARN: {cam_name} 视频写入失败，请检查 ffmpeg 输出。")
            else:
                saved_count += 1
...
def episode_output_dir_from_path(input_path: str, hdf5_path: str, output_dir: str) -> str:
    """
    目录输入时保留 config/episode 信息，避免不同 config 下 episode0.hdf5 输出重名。
    例如：
      data/beat_block_hammer/demo_clean/data/episode0.hdf5
      -> output_dir/demo_clean/episode0
    """
    if os.path.isfile(input_path):
        return output_dir

    rel = os.path.relpath(hdf5_path, input_path)
...
def process_one_episode(
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        if auto_select_moving_arm and cameras is None:
            ...
            missing_cams = [c for c in cameras if c not in available_cams]
            selected_cams = [c for c in cameras if c in available_cams]
            if missing_cams:
                print(f"WARN: HDF5 中不存在这些相机，已跳过: {missing_cams}")
        ...
        meta = {
            ...
            "requested_cameras": requested_cams,
        ...
        for cam_name in selected_cams:
            cam_g = f["/observation"][cam_name] if "/observation" in f and cam_name in f["/observation"] else {}
            missing = [
                field for field in ("rgb", "depth", "intrinsic_cv", "extrinsic_cv", "cam2world_gl")
                if field not in cam_g
            ]
            if missing:
                meta["missing_camera_fields"][cam_name] = missing
        with open(os.path.join(episode_output_dir, "meta.json"), "w", encoding="utf-8") as fp:
            json.dump(meta, fp, ensure_ascii=False, indent=2)

        for cam_name, missing in meta["missing_camera_fields"].items():
            print(f"WARN: {cam_name} 缺少字段: {missing}")

        # 保存对应手臂轨迹
        robot_dir = os.path.join(episode_output_dir, "robot_data")
        if save_both_arms:
            save_robot_data_from_hdf5(f, robot_dir, "left")
            save_robot_data_from_hdf5(f, robot_dir, "right")
        else:
        ...
        save_camera_data_from_hdf5(
            f,
            cam_dir,
            cameras=selected_cams,
            save_videos=save_videos,
            video_fps=video_fps,
            save_cam2world=save_cam2world,
            save_depth_arrays=save_depth_arrays,
        )
...
def main():
    ...
    # 批量操作
    parser.add_argument("--save-3d-data", action="store_true",
                        help="提取相机 3D 数据：head_camera/left_camera/right_camera/third_view 的 RGB、depth、intrinsic/extrinsic/cam2world")
    parser.add_argument("--save-camera-data", "-C", action="store_true",
                        help="保存所有帧的相机数据（RGB PNG, depth PNG, intrinsic/extrinsic/cam2world NPY）")
    parser.add_argument("--save-videos", action="store_true",
                        help="同时为每个选中相机保存一个 MP4 视频（需要 ffmpeg）")
    parser.add_argument("--video-fps", type=float, default=30.0, help="保存 MP4 视频的帧率 (默认: 30)")
    parser.add_argument("--save-cam2world", dest="save_cam2world", action="store_true", default=False,
                        help="额外保存 cam2world_gl 矩阵（默认不保存）")
    parser.add_argument("--save-depth-arrays", action="store_true", default=False,
                        help="额外保存 depth 原始 NPY 数组（默认只保存 depth PNG）")
    parser.add_argument("--save-both-arms", action="store_true",
                        help="保存 left/right 两侧机械臂轨迹，而不是只保存选中的一侧")
    ...
    # 输出选项
    parser.add_argument("--output-dir", "-o", default="datasets/robotwin_aloha", help="输出目录 (默认: extracted_data)")
    ...
    if args.save_3d_data:
        args.save_camera_data = True
        args.auto_select_moving_arm = False
        args.save_both_arms = True
        if args.cameras is None:
            args.cameras = DEFAULT_3D_CAMERAS

    if not args.save_camera_data:
        print("未指定操作模式，默认使用 --save-3d-data 提取 head/left/right/third_view 相机 3D 数据。")
        args.save_3d_data = True
        args.save_camera_data = True
        args.auto_select_moving_arm = False
        args.save_both_arms = True
        if args.cameras is None:
    ...
    if len(hdf5_files) == 1 and os.path.isfile(args.file_path):
        ...
        process_one_episode(
            ...
            save_cam2world=args.save_cam2world,
            save_depth_arrays=args.save_depth_arrays,
            save_both_arms=args.save_both_arms,
        )
    else:
        # 目录：output_dir 作为 root，自动创建 episode 子目录
        print(f"检测到 {len(hdf5_files)} 个 hdf5 文件，开始批量提取...")
        for i, hdf5_path in enumerate(hdf5_files):
            ep_out = episode_output_dir_from_path(args.file_path, hdf5_path, args.output_dir)
            ep_name = os.path.relpath(ep_out, args.output_dir)
