...
#   - output: SAVE_ROOT/<config>_<episodes>ep_<run_id>/<task>/episode*
#
# Examples:
#   bash batch_collect_aloha_camera_test_set_multi_gpu.sh
#   bash batch_collect_aloha_camera_test_set_multi_gpu.sh --gpus "0 1 2 3"
#   bash batch_collect_aloha_camera_test_set_multi_gpu.sh --gpus "0 1 2 3" beat_block_hammer click_bell
#   RUN_ID=seed_v2 SAVE_ROOT=./datasets/robotwin_aloha_testset bash batch_collect_aloha_camera_test_set_multi_gpu.sh
...
usage() {
  cat <<'EOF'
Usage:
  bash batch_collect_aloha_camera_test_set_multi_gpu.sh [options] [task_name ...]

Default recipe:
  Generate 3 episodes per task from task_config/custom_aloha_clean.yml with a fresh
...
Options:
  ...
  --episodes N         Episodes per task in the generated config (default: 3)
  ...
  --output-name DIR    Output subdir under save-root (default: <config>_<episodes>ep_<run_id>; use "." for save-root directly)
