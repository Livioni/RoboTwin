...
def find_npy_files(root: Path) -> list[Path]:
    return sorted(root.rglob("**/depths/*.npy"))
...
def prepare_png_array(
    ...
    # If no scale and no auto_scale, assume data is already in suitable range for uint16
    # Clip to valid uint16 range and convert
    clipped = np.clip(array.astype(np.float64), 0, 65535)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert depth .npy files to PNG with progress. Data is assumed to be in mm units."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Multiply depth by scale before saving (default: None, no scaling since data is already in mm)",
    )
    parser.add_argument(
        "--auto-scale",
        action="store_true",
...
def main() -> int:
    ...
    total = len(files)
    if total == 0:
        print("No .npy files found under **/depths/ folders.")
        return 0
